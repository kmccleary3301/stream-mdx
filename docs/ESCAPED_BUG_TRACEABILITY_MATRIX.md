...
| --- | --- | --- | --- | --- | --- |
| Empty nested list shell | Phantom nested list renders under a parent item with no committed child items | `imaginary-empty-list`, `lists-nested` | HTML harness `forbidEmptyNestedLists`; renderer empty nested list guard; committed list shell must have committed items | `imaginary-empty-list / S2_typical` seeded smoke; `imaginary-empty-list / S5_pathological_boundaries`; `lists-nested / S2_typical` seeded smoke | `packages/markdown-v2-react/__tests__/empty-nested-list-guard.test.tsx` |
| Ordered/unordered list fragmentation | Spurious list block split or imagined break during streaming | `lists-nested`, `imaginary-empty-list`, `edge-regressions` | list item count continuity; nested list depth normalization; streamed final HTML parity under seeded replay | `lists-nested / S2_typical` seeded smoke; `imaginary-empty-list / S2_typical` seeded smoke; `imaginary-empty-list / S5_pathological_boundaries` targeted replay | `packages/markdown-v2-react/__tests__/list-depth-normalization.test.ts`, `packages/markdown-v2-react/__tests__/store-reorder.test.ts` |
...
| Code block line-order or append corruption | Code content arrives incomplete, out of order, or mutates via unsafe append | `code-huge`, `code-highlight-incremental`, `mdx-math-code-mixed` | contiguous line ordering; append guard rejection; ambiguous code mutation falls back to semantic replace | `code-huge / S1_slow_small` seeded smoke; `code-highlight-incremental / S2_typical` seeded smoke | `packages/markdown-v2-react/__tests__/append-lines-guard.test.ts`, `packages/markdown-v2-react/__tests__/stale-epoch-guard.test.ts`, `packages/markdown-v2-react/__tests__/code-block-line-update.test.ts`, `packages/markdown-v2-react/__tests__/finalized-code-token-fallback.test.tsx`, `packages/markdown-v2-core/__tests__/code-fence-streaming-boundary.test.ts` |
| Premature terminal newline in streamed code | Intermediate code checkpoint is no longer a prefix of final code text | `code-huge`, `code-highlight-incremental` | `enforceCodeTextPrefix`; rendered frontier must align with source frontier before synthesizing terminal newline; partial closing fence prefixes must not materialize as code lines | `code-huge / S1_slow_small` seeded smoke; `code-highlight-incremental / S2_typical` seeded smoke | `packages/markdown-v2-react/__tests__/code-block-terminal-newline-guard.test.ts`, `packages/markdown-v2-core/__tests__/code-fence-streaming-boundary.test.ts` |
