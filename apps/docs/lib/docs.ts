...
const STATIC_DOC_SLUGS = new Set<string>(["tui-json-protocol"]);

export function getAllDocSlugs(): string[] {
  return DOC_SECTIONS.flatMap((section) => section.items.map((item) => item.slug)).filter(
    (slug) => slug.length > 0 && !STATIC_DOC_SLUGS.has(slug),
  );
}
